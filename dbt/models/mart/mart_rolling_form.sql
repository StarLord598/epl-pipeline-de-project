...
team_matches as (
    select matchday, utc_date, home_team as team, home_score as gf, away_score as ga,
    ...
    from match_results
    union all
    select matchday, utc_date, away_team as team, away_score as gf, home_score as ga,
...
with_rolling as (
    select
        ...
        avg(pts * 1.0) over (partition by team order by matchday rows between 4 preceding and current row) as rolling_5_ppg,
        avg(gf * 1.0) over (partition by team order by matchday rows between 4 preceding and current row) as rolling_5_gf,
        avg(ga * 1.0) over (partition by team order by matchday rows between 4 preceding and current row) as rolling_5_ga,
        -- Last 5 form string
        string_agg(result, '') over (partition by team order by matchday rows between 4 preceding and current row) as last_5_form,
        -- Count of matches for windowing
        row_number() over (partition by team order by matchday desc) as recency_rank
    from team_matches
)
