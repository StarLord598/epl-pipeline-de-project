...
with matchday_standings as (
    ...
    select
        cast(json_extract_string(lm.raw_json, '$.matchday') as integer) as matchday,
        max(lm.utc_date)::date as matchday_date,
        regexp_replace(regexp_replace(lm.home_team_name, ' FC$', ''), '^AFC ', '') as team_name,
        -- We need to compute standings from results up to this matchday
        1 as placeholder
    from raw.live_matches lm
...
team_results as (
    select matchday, home_team as team, home_score as gf, away_score as ga,
        case when home_score > away_score then 3 when home_score = away_score then 1 else 0 end as pts,
        match_date
    from match_results
    union all
    select matchday, away_team as team, away_score as gf, home_score as ga,
...
cumulative as (
    select
        ...
        sum(tr.pts) over (partition by tr.team order by tr.matchday rows unbounded preceding) as cum_points,
        sum(tr.gf) over (partition by tr.team order by tr.matchday rows unbounded preceding) as cum_gf,
        sum(tr.ga) over (partition by tr.team order by tr.matchday rows unbounded preceding) as cum_ga,
        count(*) over (partition by tr.team order by tr.matchday rows unbounded preceding) as cum_played,
        max(tr.match_date) over (partition by tr.team order by tr.matchday rows unbounded preceding) as matchday_date
...
with_changes as (
    select
        *,
        ...
        case
            when lag(position) over (partition by team_name order by matchday) is null then true
            when lag(position) over (partition by team_name order by matchday) != position then true
            else false
